...
describe('CartService', () => {
  ...
  describe('create', () => {
    it('should create and save a cart', async () => {
      ...
      expect(cartRepo.create).toHaveBeenCalledWith(dto);
  ...
  describe('update', () => {
    it('should update and save a cart', async () => {
      const cart = { id: 'cart1' };
      cartRepo.preload.mockResolvedValue(cart);
      cartRepo.save.mockResolvedValue(cart);
      expect(await service.update('cart1', {} as any)).toEqual(cart);
    });
    it('should throw if not found', async () => {
      cartRepo.preload.mockResolvedValue(undefined);
